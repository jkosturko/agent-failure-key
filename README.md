...
36 operational failure modes in AI-orchestrated agent systems, grounded in incident traces and operational audits.

*Jessica Kosturko · v1.0 · August 2026 · 36 modes*
...
#### What Is This
A catalog of failure modes in AI-orchestrated agent systems. Grounded in logged incidents on an instrumented agent platform the author designed and runs in daily operation, and cross-referenced with current academic research. Some of these modes are classic operations failures; they earn a place here because agent systems change how often they fire, how long they run unobserved, and whether a machine acts on the misdiagnosis.

<div class="fk-callout">


#### If You Read Five Entries

- [`#30` **Probe Inflation**](#30-probe-inflation): the observer's probes inflate the health numbers
- [`#28` **Phantom Mutation**](#28-phantom-mutation): the agent reports work it never did
- [`#13` **Wrong-Detail Mutation**](#13-wrong-detail-mutation): right format, wrong content
- [`#25` **Monitoring-Induced Rate Limit Amplification**](#25-monitoring-induced-rate-limit-amplification): the watchdog causes the outage
- [`#22` **Phantom Success**](#22-phantom-success): the agent invents the deliverable; surface-level checks grade it a pass
...
#### On the Numbering
Mode numbers are stable identifiers, not a sequence. Gaps are modes that were merged or retired between drafts, and the numbers of the survivors never move. Modes are presented by family, not by number, so numeric jumps inside the catalog are ordering, not omissions.
...
When researchers analyzed 1,600+ execution traces from seven open-source multi-agent frameworks, every system failed on a large share of its tasks: reported failure rates ran from **41%** to **86.7%**. The frameworks ran different benchmarks, so those two numbers don't rank one system against another. Even the best pairing they measured failed 41% of its tasks. Their taxonomy sorts the failures into three clusters: system design issues, inter-agent misalignment, and task verification. The third is this catalog's home ground. <span class="fk-cite">(Cemri et al., 2025, arXiv:2503.13657)</span>

And pass/fail scoring barely sees any of it. One enterprise study scored the same agents two ways and asked which score better matches how expert reviewers judged the work ready to deploy. Pass/fail landed at **0.39** (a rank correlation), where 1.0 means the score ranks the work exactly as the experts did, 0 means its ranking is no better than shuffling, and 0.39 sits closer to the shuffle. Grade the same agents on five dimensions at once, cost among them, and the number climbs to **0.81**. Some of the climb is granularity, since a pass/fail score cannot rank the work inside its two buckets. A green test run and a working agent are simply not the same claim. The space between those two claims is what this catalog maps. <span class="fk-cite">(CLEAR Framework, arXiv:2511.14136)</span>

#### Scope & Limits
This is a guide to failures of agent *systems*: models wired to tools, workflows, schedules, and each other, operating over time. It is not a catalog of model errors: raw accuracy, bias, jailbreaks, and content hallucination each have their own literatures, and this guide doesn't compete with them. Model behavior appears here only at the moment it touches the system: a hallucinated value matters to this catalog when it lands in an action, crosses a handoff, or gets recorded as truth. The catalog names the modes and the evidence that separates them. The observing platform's design appears only where a mode requires it, above all in the family where the observer itself is the failing component; otherwise it stays deliberately out of scope.

Security has its own taxonomies too, and they are among the field's most cited. Microsoft's AI Red Team catalogs 34 failure modes for agentic systems, most with a threat actor as the cause (*Taxonomy of Failure Modes in Agentic AI Systems*, v2.0), and OWASP's *Top 10 for Agentic Applications for 2026* maps the same ground for applications. This catalog is their benign complement: the same surfaces failing with no attacker anywhere. Prompt injection is excluded the way jailbreaks are, and that has a triage consequence: a wrong action born from injected content walks this catalog's checks to a benign diagnosis. When a wrong value traces to content an outsider could author, the investigation leaves this catalog for the security literature.

#### The Baseline This Guide Builds On
One failure mode is deliberately not numbered here: plain content hallucination, the model stating a wrong fact in its output. It is the industry's most-studied failure, with a comparatively mature toolchain: grounding, retrieval, fact-checking, judge models. This catalog starts where that coverage stops: the same generative fault reaching *actions*, where the output-side toolchain cannot see it. It keeps a reference entry: Appendix A.

#### Provenance
Every incident in this catalog happened on one platform: a single-operator agent fleet (chat orchestration, calendar, tasks, briefings, research, and document mutation) running continuously since early 2026, instrumented end to end, with every dispatch traced and graded. Instrumented here means: dispatch records, tool-call traces, raw provider responses where available, target-system read-backs for selected mutations, automated grading records, human ratings, incident tickets, and workflow execution histories. It does not mean universal replay, and there is no visibility into provider internals. That origin is the catalog's strength and its limit in the same breath. The receipts are first-hand: dates, error text, and trace excerpts are quoted from the operator's records, which stay private because they carry personal data. The evidence blocks are reconstructions from those records: quoted strings are verbatim; the surrounding narration is editorial. Most entries are grounded in logged incidents; where an example is a composite, a disclosed probe, or consistent with the mode rather than a proven instance, the entry says so. The frequencies are one system's history rather than an industry survey. The mechanisms and the checks are written to travel; the incident counts are not. Evidence in this catalog runs through August 21, 2026; sources were reviewed through August 2026.

**On the examples.** Many incidents use personal-domain objects because those were the logged artifacts in this fleet; the examples are small because the traces are inspectable. Each is an instance of an enterprise failure shape: unverified mutation, wrong-detail write, stale source data, false success, silent routing failure. Replace a morning briefing with a status report, a calendar with a change window, or a task list with a ticket queue; the detection problem is unchanged.
...
| 10  | [Claimed Completion Without Verification](#10-claimed-completion-without-verification)         | The call fired and returned success; the target system shows nothing changed                               |
| 36  | [Lost Update](#36-lost-update)                                                                 | A whole-file rewrite built on a stale copy erases another run's landed change; every run green |
...
| 34  | [Wrong-Action Mutation](#34-wrong-action-mutation)                                             | Asked to update a task, the agent created a duplicate instead     |
...
| 13  | [Wrong-Detail Mutation](#13-wrong-detail-mutation)                                             | The agent confirms 9am; the write that lands is 4am                                    |
| 32  | [Poisoned Handoff](#32-poisoned-handoff)                                                       | A wrong value crosses an agent handoff and becomes the next agent's ground truth; the downstream work is done right, on bad input                 |
| 14  | [Batch Scoring Dilution](#14-batch-scoring-dilution)                                           | Two actions score 100% and one scores zero; the batch reports 67%, not "two passes and a failure"                             |
| 18  | [Upstream Data Quality Degradation](#18-upstream-data-quality-degradation)                     | The agent builds its briefing correctly from what the source holds; the source held 30+ stale tasks                                               |
|     | **Cost & Observer-Self-Harm** |     |
| 30  | [Probe Inflation](#30-probe-inflation)                                                         | 16 passing probes bury 4 failing real dispatches: the success rate reads 80% while users see 0%                                                    |
...
| 23  | [Runaway Cost Amplification](#23-runaway-cost-amplification)                                   | Spend multiplies quietly while health signals stay green                                                                    |
...
| 9   | [Credential Failures (Opaque)](#9-credential-failures-opaque)                                  | Auth fails; the provider scatters possible causes and commits to none                                                          |
...
| 15  | [Model Deprecation / Autonomous Model Change](#15-model-deprecation--autonomous-model-change)  | An agent "fixes" a broken model ID with a deprecated one from stale training data                                         |
...
|     | **Capability & Eval** |     |
| 11  | [Partial Intent Fulfillment](#11-partial-intent-fulfillment)                                   | The agent understands the ask and can't act; the declared skill has no tool behind it                        |
| 16  | [Opaque Error Diagnosis](#16-opaque-error-diagnosis)                                           | The classifier reads a summary, not the error |
| 17  | [Behavioral Regression After Workflow Update](#17-behavioral-regression-after-workflow-update) | A previously guaranteed behavior breaks after an update; the routing probe still passes HTTP 200                                 |
...
| 20  | [Shared-Dependency Blindspot](#20-shared-dependency-blindspot)                                 | Four layers correctly detect their own local symptom; none surfaces it as a common error                            |
...
**Appendix A**: [Hallucination](#appendix-a-hallucination): the baseline failure, kept for completeness; a common parent mechanism behind #28, #13, and #32.
...
Start at the top with what you asked for, then follow what you saw from your own chair. An ask that is both an answer and an action is two claims; walk each path once. The legend on plate 1 covers all nine plates; the numbered cards point into the catalog below. On the plates, long mode names are shortened for space: Junk Source is #18 Upstream Data Quality Degradation, Stale Data is #5 Stale Data Presentation; the mode number, not the label, is the identifier to match on. Solid arrows are the next check to run now. Dashed arrows are later moves: a deeper look surfaces something hidden, and a case that wore one label migrates to its neighbor, the same one-way move described under #28. The key walks the false-success question specifically; roughly two dozen of the catalog's modes never appear on it.

<div class="fk-key-page">

[![The False-Success Key, plate 1 of 9: cover, legend, and the top fork between the answer path and the action path](taxonomy-assets/false-success-key-01.png)](taxonomy-assets/false-success-key-01.png)
...
[![The False-Success Key, plate 2 of 9: the answer path, from claimed success to honest non-claims, hallucinated content, and the code side path](taxonomy-assets/false-success-key-02.png)](taxonomy-assets/false-success-key-02.png)
...
[![The False-Success Key, plate 3 of 9: the action path, sorting the four visible symptoms of a wrong-looking result](taxonomy-assets/false-success-key-03.png)](taxonomy-assets/false-success-key-03.png)
...
[![The False-Success Key, plate 4 of 9: the question 'where was the wrong detail born?', from invented values to a wrongly declined worker](taxonomy-assets/false-success-key-04.png)](taxonomy-assets/false-success-key-04.png)
...
[![The False-Success Key, plate 5 of 9: the question 'check the trace: which tool fired?'](taxonomy-assets/false-success-key-05.png)](taxonomy-assets/false-success-key-05.png)
...
[![The False-Success Key, plate 6 of 9: the question 'check the trace: was there an error?', and where the change went](taxonomy-assets/false-success-key-06.png)](taxonomy-assets/false-success-key-06.png)
...
[![The False-Success Key, plate 7 of 9: the records door, for results that look like success](taxonomy-assets/false-success-key-07.png)](taxonomy-assets/false-success-key-07.png)
...
[![The False-Success Key, plate 8 of 9: the deeper-investigation targets that modes dash-point to](taxonomy-assets/false-success-key-08.png)](taxonomy-assets/false-success-key-08.png)
...
[![The False-Success Key, plate 9 of 9: the index of modes on the key](taxonomy-assets/false-success-key-09.png)](taxonomy-assets/false-success-key-09.png)

</div>

### Two rules the key is built on

**Every road to verified success runs through the records.** The key never lets an outcome that merely looks right count as verified; looking right is filed as trust, often an acceptable risk and never a verified state. The two tempting shortcuts don't hold. Automated graders reading an agent's reply barely separate real success from false, topping out at **0.65** AUROC, where 1.0 is perfect separation and 0.5 is guessing <span class="fk-cite">(Advani, 2026)</span>. And a check on the world's end state can be fooled too: across the benchmark settings of one procedure-aware study, **27% to 78%** of nominally successful runs concealed violations in how the work was actually done <span class="fk-cite">(Cao et al., 2026)</span>. Only records settle the question, and each kind answers its own half: the trace, what was attempted; the read-back, what the world now holds. An action that matters deserves both.

**The honest paths exist only where someone built them.** A model can say "I don't know" only if abstention survives its tuning. An agent can say "already done" only if the reply format has the phrase. A router can say "no worker fits" only if the choice isn't forced. This recurs everywhere the key branches: where a layer has no honest way to state its true condition, fabrication or forced action becomes its nearest available behavior. Several modes in this catalog are exactly that substitution, seen from downstream.
...
| You set up a backup; the moment the primary failed, the backup failed too, for a different reason | [#21 Phantom Fallback](#21-phantom-fallback) | Send one real request through the "healthy" backup path right now. It fails (unfunded, expired, revoked) even though the vendor's status page is green → #21; health only checked that a key was present. The vendor itself is down → ask what the two paths share. Primary and backup sit on the same vendor, account, or key → [#20 Shared-Dependency Blindspot](#20-shared-dependency-blindspot); they do not → two independent outages, not this mode. |

### There is an error, and it tells you nothing useful

| What you see | Likely mode(s) | The check that splits them |
|---|---|---|
| Every call comes back "unauthorized" with nothing else to go on, and no recent deploy | [#9 Credential Failures (Opaque)](#9-credential-failures-opaque) · [#16 Opaque Error Diagnosis](#16-opaque-error-diagnosis) | Pull the raw provider response for one failing call. The raw text names the cause plainly and only a summary buried it → #16. The raw text commits to no cause (a bare 401/403, or boilerplate listing several possibilities), and a human re-authenticating fixes it in minutes → #9. |
...
| An agent that ran clean for weeks fails on every run starting one morning, with no deploy and no edits | [#9 Credential Failures (Opaque)](#9-credential-failures-opaque) · [#15 Model Deprecation / Autonomous Model Change](#15-model-deprecation--autonomous-model-change) | Pull the raw response for one failing call. An auth error naming no actionable cause, fixed in minutes by a human re-authenticating in the provider's console → #9. The error names a missing model or API version rather than "unauthorized" → #15. |
...
If no row fits, go back to the [mode index](#mode-index) at the top and read the closest family straight through.

</div>

## The 36 modes

**Modes stack, and some modes mask.** Most failure modes are properties of one segment of a run; a handful live instead in the machinery that interprets runs: scoring, retention, records, coverage. A single run crosses many segments, so real incidents often carry several modes at once; one night in this catalog's history produced three inside thirty-one minutes, the first two seventeen seconds apart. Worse, some modes hide others: synthetic traffic conceals what breaks real traffic (#30), a prettified error hides the diagnosis that would have named the cause (#33), and an unverified "done" hides everything downstream of it (#10). That is the working argument for layered detection: no single check sees a stack. Checks run before dispatch, in flight, and after the fact, each independently strong so a gap in one is covered by another. One design choice underlies the whole catalog: the layer that observes agents never performs their work, so the checks stay honest about systems they have no hand in running. Together they show the throughline of this catalog: a system that looks green is not the same claim as a system that works.

**How each entry is built:** every mode carries **What**, **Why it's dangerous**, **The principle**, and at least one evidence field: an **Example** (composite or illustrative, labeled when so), an **Observed incident** (a logged case from the fleet), or both. Three fields appear only where they apply: **On the key** for the fourteen modes the False-Success Key routes to, **Grounding** where the evidence needs context beyond the incident itself (how it was elicited, or what the research adds), and **Relationship to other failure modes** where a real relationship is documented. Absence of a conditional field is by design, not omission.
...
**What:** When an agent tells you it performed an action (updated a task, edited a file), the report and the act are two separate events. Phantom Mutation is when they come apart at the source: the model composes a confident description of work it never did, because the tool was never invoked. Nothing errors: a call that never happens has nothing to throw. Every automated layer that reads only the agent's words agrees with the agent. The only witness that disagrees is the thing that was supposed to change. The research literature's "tool hallucination" family describes emitted calls gone wrong: a nonexistent tool, the wrong tool <span class="fk-cite">(Xu, H. et al., 2024)</span>; trace taxonomies file invented tool outputs and claimed nonexistent capabilities under one coarse heading (TRAIL). This mode is the case that family leaves out: no call is emitted at all; only the claim exists.

**Example:** Mid-afternoon, a user asked a packing assistant to add a rain coat to a travel packing list. Ninety seconds later, the same ask came again. The first request was real end to end: a dispatch fired, the agent read the file, and the item landed. The second reply claimed the same success:

```
"I've successfully added 'rain coat' to your [travel] packing list."
...
This is one proven instance, and a limited one: the actor was the coordinating layer rather than the packing agent, and the phantom turn wrote nothing, so it destroyed nothing. It remains the example because the mechanism is fully documented: a repeated request was answered from conversation memory, the fabricated report even included a perfect self-assigned score that the automated review accepted, and the missing call is provable because the adjacent runs establish the recorder was working.
...
- The inviting conditions are ordinary, and research has begun measuring them: models skip a required tool 12 to 29% of the time depending on the model, a mean of about 19% across the 19 models on the published leaderboard, on tasks built so a memorized answer competes with the tool return <span class="fk-cite">(ToolFailBench, Soni, 2026)</span>, and training can teach convincing over correct, with humans approving wrong answers up to 24 points more often while real correctness barely moved <span class="fk-cite">(Wen et al., 2024)</span>. A grader that reads words completes the set: a judge that can inspect the actual artifacts aligns with human judges far better than one that only reads the output, about 90% versus 70% <span class="fk-cite">(*Agent-as-a-Judge*, Zhuge et al., arXiv:2410.10934)</span>. None of this is rare; a long-running assistant accumulates all three.
...
- **#36 (Lost Update):** the other way a promised change ends up missing. To the person affected the two present identically: the agent said done, and the world lacks the change. The change history splits them: after a phantom, no version ever held the change; after a lost update, one version held it and a later write destroyed it. (This entry's example and #36's are one incident: same file, same afternoon.)
...
**What:** An AI agent reports "done" but didn't verify the outcome. Nothing confirmed the effect: the action may have failed downstream, errored quietly, or changed nothing. What separates this mode from #28 is the trace: #28's trace is empty, no call and no error, only the claim; here the trace holds a call, sometimes ending in a buried error the claim papered over, sometimes in a clean OK, and either way the claimed outcome was never checked against the target system.
...
**Why it's dangerous:** Self-reported success is narrative, not measurement. Anthropic's eval guidance (docs.anthropic.com) recommends objective, automatable grading (code-based checks combined with model-based rubrics), and its agent-evals guide (anthropic.com) grades outcomes with independent tests rather than the agent's transcript alone. The VIGIL reflective runtime <span class="fk-cite">(Cruz, 2025, arXiv:2512.07094)</span> demonstrates meta-level self-repair via a state-gated pipeline in which illegal transitions surface explicit errors rather than letting the agent improvise past them, the same discipline that separates a verified outcome from a self-reported one.

**The principle:** For any dispatch whose outcome matters, independently confirm the outcome: for a mutating one (create, update, delete), query the target system and check the side effect occurred. Dispatch success ≠ outcome success. This is the middle rung of the verification ladder: the trace check passes here (the call did fire, in this mode's central case); only reading the outcome back catches it (#28, #13 (Wrong-Detail Mutation) are the rungs beside it).
...
**Example (traced; proves the loss, not the eraser):** One packing list, three runs, four minutes.

```
── 2:42 PM   THE ADD ───────────────────────────────────────────
1. Run one adds a rain coat to the section for the
   upcoming trip and writes the file ✓
...
── 2:44 PM   THE LOSS ──────────────────────────────────────────
4. Run two reads the file: 3,106 characters. That
...
13. For over a minute and a half the file contradicted
    a receipted success
```

Each run, examined alone, is correct and complete. The erasure belongs to none of them: it happened in the untraced gap, under two minutes, between run one's write and run two's read.
...
**Observed incident:** May 2026, on a travel packing list, traced above. The operator saw the symptom the same afternoon: five asks for the rain coat that day, and the restore run was the operator's re-ask. The fourth ask was the phantom traced in #28 (Phantom Mutation), the only chat turn between run one's write and run two's read; it made no file call, so it is not the eraser. The why stayed hidden for three months, until a forensic sweep lined up consecutive file versions by size and content. No run's own status, and no check run after the restore, could have shown that; the change history did. The history proves the loss, not the eraser, so this incident is consistent with the mode rather than a proven instance of it.

The class is documented outside this catalog, though only recently and mostly by people building verification tools rather than cataloging failures. A 2026 formal-methods study modeled agent shared state as long-running read-generate-write operations, specified the resulting anomalies in a proof language, and reproduced a silent lost update in a widely used open-source agent framework (Khan, 2026). A coordination study measured the same thing from the other side: a file-based tracker loses concurrent writes, while an append-only shared log converged with no write silently dropped (Sarkar, 2026). Work on agent memory has begun importing the database vocabulary directly, observing that production systems pick a conflict-resolution heuristic without declaring which write-time anomalies it admits (Wang, 2026). The agent failure taxonomies reviewed for this catalog have not picked it up: the security-side catalogs reviewed here carry no concurrency concept, and the nearest fault-study entry is a small race-condition bucket whose symptoms are loud <span class="fk-cite">(Shah et al., 2026)</span>. One of them names a neighbor, artifacts reverting after context truncation, which is a different cause with the same shape.
...
**What:** An agent that can't do what it was asked often doesn't say so. The model is built to be helpful, so it fills the gap instead of surfacing the error: a briefing with confident dates, a schedule for a day it knows nothing about, an event with invented details. Usually nothing in the output warns you it isn't real: it reads well, nothing threw an error, the right words appear. When an admission does slip in, it sits mid-deliverable or deep in the trace, where no check reads it before the deliverable lands. Either way you take it as correct. The operator finds out by reading, not from any alert.
...
**The principle:** Verify outputs against reality, not plausibility: do the dates match today, does the data trace to a source, did the empty case come back as an honest "nothing to report." A model that can't say "I have nothing" will always have something; that is what checks are for. And any declared check that can't run must fail loud, never skip silently. Read the deliverable, too, for the admission the model sometimes buries in it: a line naming the missing input, an aside that no data came back, a sentence explaining why the answer cannot be right. A cheap grep for a handful of these phrases catches the cases where the model wrote its own defense into the output.

**Observed incidents:** March 2026, twice in one month in this mode alone. The fake-briefing morning traced above; the same-day fix made the missing checks real and unknown check types fail loud. Two weeks later, a calendar agent that found no events invented one: a fabricated meeting with invented details, dated the wrong year; the same-day fix paired a stronger model with an explicit empty-result guard, so "no events" comes back as "no events" (Appendix A reads this same incident at the hallucination layer). The mechanism has an independent name now: a production study of a single-agent runtime calls it fail-plausible, the model converting a failure into a fluent, plausible narrative, and logs four incidents of its own <span class="fk-cite">(Wu, 2026)</span>. The scale is measured now: on a customer-service agent benchmark, 45 to 48% of all failures were false successes, the agent asserting completion the environment's state disproves <span class="fk-cite">(Advani, 2026)</span>. And the confessed variant has a documented instance in the wild: a frontier model wrote in its own reasoning trace that it could fudge the tests by making verification always return true, the tests recorded a pass, and monitors reading only the actions caught barely more than half of such hacks <span class="fk-cite">(Baker et al., 2025)</span>.
...
- The coordinator is often the least-instrumented model in the stack. Teams instrument the workers and grade their outputs, while the selection step upstream frequently carries no check of its own. The research that tries to reconstruct blame after the fact reports how costly that gap is: automated methods name the responsible agent barely half the time <span class="fk-cite">(Who&When, arXiv:2505.00212)</span>.
- The pattern is measured, not speculative: in an orchestration benchmark built from deliberately confusable options, wrong selection among semantic lookalikes is the dominant first failure, 50 to 58% under four of the five planning methods tested (32% under the fifth), explicitly separated from its downstream cascade <span class="fk-cite">(Chen et al., 2026, arXiv:2605.27957)</span>.
- Blame lands on the worker by construction: downstream steps inherit the corrupted assignment and look guilty. State-of-the-art attribution methods locate the true root step less than 15% of the time on the Who&When benchmark <span class="fk-cite">(Ma et al., 2025, arXiv:2509.08682)</span>, and even a model built for attribution reaches only about 21% on hand-crafted failures <span class="fk-cite">(Rafi et al., 2026, arXiv:2606.00765)</span>.
- Selection confusion among similar candidates is a measured weakness, at least for tools: models "still struggle to effectively select tools" when the choices resemble each other <span class="fk-cite">(MetaTool, arXiv:2310.03128)</span>. A study testing confusable agent *names* has yet to turn up. A roster full of look-alikes is the same trap one level up.
...
**Observed incidents:** March 13-14, 2026, traced above, with the full chain preserved in the records: the wrong agent's capability text captured inside the router's own injected prompt, two false-green dispatches on consecutive days, the operator's re-issued requests as the human label, a filed bug naming the misroute, and the fix commit that edited two description lines. Bug and fix landed two weeks after the false-greens, when the same collision struck a third time and finally failed visibly. Two same-platform siblings carry neighboring sub-modes: a request that named one worker was answered by a lookalike sibling with a confident report on an unrelated topic while the named worker sat healthy (June 2026), and a router silently substituted a different worker for the one named, admitting the swap in its own reasoning (March 2026). Three sub-modes (description collision, name collision, silent substitution), not three replications of one mechanism; all evidence is one operator's platform.

The wider field measures this without naming it. One enterprise study benchmarks dynamic agent routing above 90% accuracy with a false-agent-switching rate under 3% <span class="fk-cite">(Shu et al., 2024, arXiv:2412.05449)</span>, so the wrong-worker rate is already a published number. A dedicated routing benchmark scores selection over a 12-agent catalog <span class="fk-cite">(Bala et al., 2026, arXiv:2606.28925)</span>. A practitioner reports the failure in the OpenAI Agents SDK's tracker, noting that model-driven handoffs mean the model "may choose the wrong agent", and asks for deterministic handoffs as the fix (openai-agents-python issue #1638). The nearest cell in the taxonomies reviewed for this catalog is Delegation Failure, scoped to incorrect scope and dependencies <span class="fk-cite">(Raj et al., 2026, arXiv:2607.28802)</span>; wrong *target* is implicit there, not its own entry. A fault study has a named improper-task-delegation category, and its instances break loudly <span class="fk-cite">(Shah et al., 2026)</span>; the silent dispatch that succeeds in the wrong shop is not among them. So: the field measures routing accuracy as a performance number, and the taxonomies reviewed here have yet to name its complement as a reliability event.

**On the key:** the "only when a worker was chosen" branch off an honest refusal (plate 3), and "the coordinator chose the worker" on the trace plate (plate 5); see [The False-Success Key](#did-it-actually-happen-the-false-success-key).
...
**The principle:** Capture what the person wanted as a record of its own, and compare the delivered outcome against that record instead of against the request text. It is the one artifact no amount of instrumentation can produce, because only the requester can author it: a restatement of the goal, confirmed before work starts, stored with the run. The pre-execution counterpart is ambiguity detection. When an ask carries two readings, the cheapest correct action is a question, and both halves of that have research behind them: agents largely fail to notice underspecified requests on their own, and interaction recovers much of the lost performance for the best models (up to 89% of fully-specified performance), though a wide gap remains for others <span class="fk-cite">(Vijayvargiya et al., 2025, arXiv:2502.13069)</span>. There is formal work on when a question is worth its cost <span class="fk-cite">(Suri et al., 2025, arXiv:2511.08798)</span> and on capturing implicit intent before execution <span class="fk-cite">(Qian et al., 2024, arXiv:2402.09205)</span>.

**Grounding:** The probe battery behind the incident above: seven deliberately ambiguous research asks, dispatched and logged like any other traffic, each with the operator's intended reading declared in advance. Five times the agent's chosen reading matched the intent; one run was excluded for a capitalization confound the operator flagged; the transformers ask was the one that landed. Across all seven asks, and three unregistered ones before them, the agent never once asked which reading was meant. The published instances are unusually clean. In text-to-SQL, a benchmark of ambiguous database questions found that models prompted without examples miss at least one valid reading on more than 98% of ambiguous questions, even when the prompt warns the question may be ambiguous and asks for a query per interpretation; the best model covered only about 31% of the valid readings, and on one ambiguity type it picked the same reading more than 97% of the time <span class="fk-cite">(AMBROSIA, arXiv:2406.19073)</span>. In a block-building task whose instructions omitted a color or a count, the most question-averse model asked almost nothing and guessed instead, capping its accuracy at 73.1%, while the two models that asked reached 85.6% and 89.4% <span class="fk-cite">(Bila et al., 2026, arXiv:2603.19997)</span>.

What the reviewed literature does not supply is the name. The closest artifacts are a triage tree over evaluation scores and several cause-first catalogs. MAST's "fail to ask for clarification" <span class="fk-cite">(Cemri et al., 2025, arXiv:2503.13657)</span> names the omitted remedy rather than this failure; the nearest match, an "instruction-grader mismatch" cell <span class="fk-cite">(Raj et al., 2026, arXiv:2607.28802)</span>, pins the fault on the requester's instruction as judged against a grader; it does not name the agent's wrong reading of an ambiguous ask. Microsoft's red-team taxonomy names a benign neighbor, misinterpretation of instructions, an agent misreading the words and acting on the misreading; this mode is the case that survives that filter: a legitimate reading, so every check on the words passes. The examples are common in the literature; the label is not.
...
6. The user reports the dispatch as "hanging": a failure the system had already caught ✗
...
**The principle:** The fix is not either/or: the operator needs the full ugly error, the user can have the clean one, and the catch site should never be the place that chooses for everyone. When scrubbing is unavoidable (secrets or personal data in the error), do it by explicit rule rather than judgment, and check the output against the input: a redaction layer is a rewrite layer, and this mode is what it becomes when it takes more than the secret. The point is preserving the diagnostic meaning.
...
- **#10 (Claimed Completion) and #22 (Phantom Success):** #10 replaces the failure signal with success; #22 may leave the admission in the output where no check reads it. Here the failure signal was caught, then stripped at the catch site.
...
**Observed incidents:** A recurring pattern in AI-assisted development on this platform, strong enough that two standing engineering rules were installed against it ("never discard error messages"; "no hidden errors, ever"). A July 2026 audit of the operator console then found four instances in one sweep, each with a dated file-and-line record, among them an error path whose comment promised an undo that nothing performed, and a resolve handler that swallowed its failure into silence. An AI assistant stamps a bad error-handling pattern everywhere it touches, fast.
...
2. The write succeeds; read back, the calendar holds 4:00 ✗
   (04:00 Eastern is 09:00 UTC exactly, start and end both: the
    announced hour read in the wrong clock. Not a display quirk;
    the stored time itself is wrong. The model assembled the time,
    and the write schema had no timezone field, so the hour it
    sent traveled with no frame on it.)
...
**The principle:** Check what the agent said against what it sent: the chat text and the API payload come from the same model in the same breath, and divergence between them is the cheapest tell this class offers. (A check you have to build on purpose: the chat text and the payload never meet in one place unless something puts them there.) The same comparison, widened to the whole context, catches the ignored-value case for free. The comparison has a limit, and the example above sits inside it. Said and sent agreed; the hour traveled with no frame on it, and the wrong value was born where the API read it. A said-against-sent check has nothing to catch there. What remains is the read-back, comparing what landed against what was said. Reject values that can't be right before the write: a brand-new event starting last year, for instance. Give the model and its tool calls the values they need instead of leaving them blank: a parameter left empty gets filled from the model's training. The clearest case is the date; a model that is never given the current date has nothing to fall back on but the years it was trained on.
...
**Why it's dangerous:**
- Traditional systems validate at the perimeter and trust the inside, and for thirty years that held because the inside was deterministic: code doesn't guess values in transit. Agent pipelines break the assumption quietly: every hop contains a generator, so a brand-new value can be born mid-pipeline, downstream of every gate. Each handoff is a fresh edge, and the gates tend to exist only where writes happen.
- Precision reads as authority. The downstream agent has no way to know a timestamp was minted from the word "Thursday," and no reason to ask.
...
**Relationship to other failure modes:**
- **#13 (Wrong-Detail Mutation):** the wrong value is born inside a single agent's turn, and the checks at that agent catch it, comparing said against sent or reading the landed state back when the two agree. Here the value is consistent everywhere downstream; the fabrication happened a hop earlier, and every said-versus-sent check passes.
- **#18 (Upstream Data Quality Degradation):** the source data was junk; here the source was fine, and the handoff invented the junk.

**Observed incident:** July 2026. An ask that named only a weekday, "on Thursday," left the orchestrator to fill in the rest: it dispatched the downstream agent with a start date of 2025-05-22, month and year manufactured from that one word. The agent did excellent work for that date, down to season-appropriate suggestions for a May more than a year in the past. The automated review scored the run perfect and praised the seasonal reasoning. The operator caught it by reading the output. A retry with the month spelled out fixed the month and kept the wrong year.
...
3. One verdict for the whole message: 0.35, partially fulfilled, filed as low ✓
...
**The principle:** Consider scoring at the grain of the ask rather than the message, giving every requested action its own verdict. When several agents share a response, give each its own report card. Where it's relevant, include actions that don't exist yet, filed under the name the user asked for. A missing capability then shows up as a clean row of zeros with a request count attached: a build case, not a vague sense of mediocrity.
...
- **#25 Monitoring-Induced Rate Limit Amplification:** both are observer-caused, but #25's monitoring *breaks* the system under observation; Probe Inflation's monitoring *misreports* it.
...
**What:** In traditional software, watching a system is nearly free: a ping, a status code. With agents, a free ping proves almost nothing (#30 shows how little); any glance deep enough to matter is a billed API call, drawing on the same rate limits and budget as the agents it watches. Most days that coupling is invisible; a few probes an hour vanish into the budget. It surfaces the day a rate limit appears. Some monitors are built to poll harder during trouble (classic monitoring suites switch to a faster retry interval once a check starts failing); others simply poll often because the agent is high-stakes. Either way the monitor now consumes exactly the resource the provider is refusing, and the loop closes: more polls produce more 429s, more 429s look like deeper degradation, so the monitor polls harder still. There is no natural exit. A partial failure becomes a total outage, with the observer as the cause.
...
2. The free key returns 429: as it had on every check since the key was added two days earlier; its
...
3. The monitor reads that everyday 429 as fresh degradation →
   switches to aggressive mode, polling every 60 seconds (a recovery feature shipped thirty-one hours earlier) ✗
...
9. ~4pm: the operator notices the 429s in the provider's console
10. ~8pm: a backend restart resets the monitor; the storm breaks
```

**Why it's dangerous:**
- It is a feedback loop with no natural brake, and the pattern is old enough to have its own section heading: the Google SRE Book's Ch. 22 warns, under *Stop Health Check Failures/Deaths*, that "health-checking itself makes the service unhealthy." What changed is the meter. The paper that named metastable failures <span class="fk-cite">(Bronson et al., HotOS '21)</span> examined failure-detector-driven failover and ruled it non-amplifying on its own, because each rerouted request is processed only once. That accounting does not price the checks themselves, and it holds right up until every check is a billed API call.
...
**Why it's dangerous:**
- Traditional SLOs have no seat for cost. Uptime, latency, and error-rate promises can all hold (five nines, perfect answers) while every request quietly wastes money the business gets nothing back for. No alert fires, because nothing was promised. Cost-per-task indicators are starting to appear in agent-SRE tooling; the typical stack still runs without one.
...
- Cost is one of five evaluation dimensions in the CLEAR framework <span class="fk-cite">(CLEAR Framework, arXiv:2511.14136)</span>. The other four have obvious dashboards; cost tends to be left to whoever reads the invoice: external, and weeks late.
...
7. Verdict: the root cause was wrong, the event count was wrong, and the
   "unrecoverable" data had survived: the human ratings in a table the
   loss never touched, the rest upstream in a system nobody had
   configured to delete anything ✗
```
...
**The principle:** Treat incident reports as living documents: when later evidence contradicts one, append a dated correction in the same wave, or mark it superseded. An unmarked stale record is a landmine with a long fuse. Ground claims about the past in artifacts (the code and data as they existed at the time) rather than reports; a report is testimony from a particular day, and it should carry its date the way testimony does.
...
**What:** An agent runs on borrowed identities: API keys and OAuth tokens, each of which dies on someone else's schedule: an expiry timer, a rotation, a revocation. When one dies, nothing in the code changed and the last deploy was fine, yet every run fails. The cheapest fix in operations gets some of the worst repair times.
...
2. A credential the agent runs on died on the provider's side, most likely its expiry timer ✗
...
- The agent should never be able to read the secret (anything an agent can read, it can leak). Rotation tooling can renew a machine credential without exposing it; where that tooling doesn't exist, as here, the path ends at a human. The platform either walks them there or leaves them to dig.

**The principle:** Name the failing step and the failing credential, link the human straight to the fix, and pass the provider's raw error text through to the operator, unsummarized. Attribution is a job for tooling; holding the credential is not. Whatever layer watches your agents should walk the human to the fix, and no layer should be able to read the secret itself.
...
- A model swap is also a **price and ROI change**: tiers within one model family can differ enormously in per-call cost (in one case measured in 2026, ~20×). A swap the operator never saw rewrites the cost structure with it.
...
**What:** A model provider will stop serving an agent because a spending limit was hit: a daily cap or a monthly budget. Nothing else about the agent changes. The webhook still answers, the workflow still runs its steps, and the one call that needed the model comes back with a cap error instead of an answer. The agent is running "brainless." By default a web server does not shut down because you spent too much (a cloud budget alerts; it does not stop the machines), so a standard runbook has no reason to say "check billing." And if the workflow is configured to continue past a failed step, the run still finishes and still counts as a success.

**Example:** A daily-briefing agent kept being dispatched with its provider's spending cap tripped:

```
March 31, evening: the provider's spending cap trips, spend over the
limit by about a quarter. Model calls now come back with a
clear 429: "Your project has exceeded its spending cap."

── THE RUNS UNDER THE CAP ──────────────────────────────────────
1. A briefing dispatch goes out; the webhook answers ✓
...
- **#29 (Orphaned Error):** a tripped cap throws a real, recorded error, and wiring that continues past it to a success surface buries it; that burial is #29. Not #1, where no error exists to lose; a cap trip always leaves one.
...
**What:** API clients reason in HTTP status codes, and model providers speak that language in dialects. The HTTP spec has no code for a spending cap or a filtered response, so each provider maps those states onto whatever existing code sits closest, and no shared convention has emerged. A 429 can mean a rate limit that clears in a minute or a spending cap that waits for a human. A 200 can mean success or a filtered response with nothing usable in it. A system that classifies failures by status code alone (a reasonable first pass for traditional APIs, where finer detail lives in the response body) will misclassify a meaningful fraction of model failures: in this catalog's comparison of provider error catalogs, spending caps, billing failures, content filtering, model overload, and tool-schema errors each wear different codes depending on who returned it.
...
**Example:** An audit walked the dashboard while its backend served a seven-week-old build: reachable, health-check green, and missing nearly every route the dashboard needed:

```
the backend behind the fleet dashboard serves a seven-week-old
build; health answers 200, nearly every data route 404s

── WHAT THE AUDIT FOUND · surface by surface ───────────────────
1. In a browser tab that has stayed open since before the routes
   broke: agent cards render the last snapshot ever fetched,
...
3. On a fresh page load, weeks in: "Downtime: 30s": the counter started
   when this load first noticed, not when the routes actually broke ✗
...
**What:** A scheduled agent can fail by producing nothing. When its trigger dies (a container restart, a lost scheduler registration) there is no failed run to report, because there is no run at all. And when runs keep firing but keep failing, every alarm grades a run that happened; nothing measures how long it has been since one succeeded. A dashboard that computes health from the last known success keeps showing that success, aging quietly. Both facts are true at once: the last success is real, and the agent has produced nothing all week. A status built only on "last result" cannot tell those two worlds apart.

**Example:** A calendar agent's runs stopped succeeding on a Monday afternoon. For four days, no signal said the agent had stopped succeeding:
...
3. Four days pass. Health computed that way stays green the
   whole time ✗
...
5. Other alarms were ringing about this agent that week, but each
   graded runs that had happened. Only the new check asked when
   the agent had last succeeded ✗
```

**Why it's dangerous:**
- The failure is an absence of success, not an event. A dead trigger emits no errors, ever, so alerting keyed to error events has nothing to fire on. Failing runs do emit errors, but each alarm grades a run that happened; none asks why success stopped arriving.
- The green status is technically true (the last success is real), so it survives a spot-check. Only a cadence question ("when should this have run again?") exposes it.
- Binary status hides it by design: the Beyond Task Completion framework <span class="fk-cite">(Akshathala et al., 2025, arXiv:2512.12791)</span> argues that binary task-completion metrics miss operational dimensions, evaluating agents across four pillars (LLMs, memory, tools, and environment) instead, because a system can pass its task check while degrading on dimensions the binary status never surfaces.

**The principle:** Every scheduled agent declares its expected cadence, and health combines last result with time since last successful run measured against that cadence: overdue past a tolerance means degraded, no matter how green the last run was. Measure the symptom, not the mechanism: "did output appear on schedule," instead of "does the scheduler's own log show a fire." And a cadence check must say which ways of starting a run it can see, because an agent's work can start from several places (a schedule, a chat request, a manual click), and the ones the check can't see look identical to death, in both directions: real work through an unwatched path reads as silence, and a watched path gone dark reads as a dead agent. Success here means the workflow's own success signal arriving on cadence; a run that succeeds while its output never reaches the reader is a delivery failure, a different hunt.

**Relationship to other failure modes:**
- **#5 (Stale Data Presentation):** the run happened and the screen presents old results as if they were fresh. Here, successful output stopped arriving by its expected cadence: the trigger may have died, or runs may be firing and failing.
...
*Scheduled runs fail where the AI layer can't see; so does anything else that starts without it.*
...
**Relationship to other failure modes:**
- **#6 (Schedule Staleness):** no successful output arriving by its expected cadence, and last-result health unable to see it; this mode is the run happening, failing, and logging its error where the AI layer never looks.
...
── THE COST ────────────────────────────────────────────────────
8. Days later a trip update fails downstream, because the
   search that should find the event cannot ✗
9. The diagnosis, written that week: "not properly wired to pass
   date/search parameters to the [calendar service] integration" ✗
```
...
**The principle:** Give a request with no route a loud exit instead of a quiet one. An explicit fallback for unmatched requests turned this mode's next occurrence into an error that announced itself in under a third of a second. For routed requests, assert that required request fields survive the hop into the downstream call.
...
- **#3 (Expression Evaluation Bugs):** also silent, also code, split by ownership. #3 lives in the engine you run on (the framework's code, where your review doesn't look); this mode lives in the code you own, where reading your own wiring can find it.
...
*The engine runs your correct config wrong.*

**What:** The agent framework runs correct logic incorrectly: an expression, routing condition, or template that reads unambiguously produces the wrong result at runtime, whether the engine evaluates it wrong or delivers its result wrong. The bug lives in the layer beneath the user's code, not in anything the user wrote.
...
**What:** An AI agent writes through an API or a Model Context Protocol (MCP) tool (a configuration change, a rewiring, any programmatic edit) and submits a value that should be impossible: outside the field's small set of valid values. The API accepts it without error. Nothing fails at write time; the invalid piece is simply ignored at runtime. The write was corrupted silently, by a caller that was never told no. The server half of the mechanism is documented at scale: a taxonomy built from 837 runtime faults in MCP servers names parameters "accepted but not enforced at runtime, leading to unintended default behavior" <span class="fk-cite">(Owotogbe et al., 2026, arXiv:2606.05339)</span>.
...
- Planning and execution are separate questions. The Agent GPA framework <span class="fk-cite">(Snowflake/Stanford, arXiv:2510.08847)</span> evaluates goal-plan-action alignment with a factorized suite of LLM judges rather than a single binary pass/fail, because one number collapses exactly this distinction.
...
2. Not that. The failed workflow answered its caller with
   a plain HTTP 500, which is all the next layer saw. Its
   one-sentence summary of the run: "Failed to query
   calendar events due to HTTP 500 error in workflow.
   Zero fulfillment of user's need to check schedule."  ✗
...
- It's a classic ops failure. Catch-all handlers and vague alerts predate AI by decades. What's new is the environment: agents fail unwatched, error vocabularies churn faster, and increasingly a machine acts on the misdiagnosis. The agent-error literature has since named the model-side case causal misattribution, noting the failure but blaming the wrong cause <span class="fk-cite">(Zhu et al., 2025)</span>. This mode is that failure with its cause on record: the classifier read a summary of the error, never the error.
...
**What:** A previously-fixed agent behavior breaks again after a workflow update, restore, or tool-layer modification. The old bug need not come back for the behavior to break again: a different change can break the same promise a new way. What regressed is the promise, not the bug. The synthetic probe returns HTTP 200 (routing is fine) but the actual behavior has regressed: content goes to the wrong output, the response is empty, or side effects fire when they shouldn't.

**Example:** April 1: a briefing regression. The same symptom had been logged, and fixed, once before. Same date as #26 (LLM Cost Cap Exhaustion), but a separate failure in the same pipeline. There the model produced nothing. Here it produced the briefing, and the response path lost it on the way back; the model path was serving again by the time this one was diagnosed.
...
3. Root cause this time: a missing timeout setting turned the call
   async, and the chat-side model never saw the content. Fixed the same day:
...
7. Nothing forced a checked run at the moment of risk, right after
   an update; grading of real traffic can catch the miss later ✗
```
...
- The failure itself is traditional distributed-systems territory; reliability engineering calls the underlying event a common-cause failure. In the traditional version, the shared dependency might be your cloud provider: if it goes down, your whole business goes down, and for many non-critical systems, single-cloud is still treated as an acceptable tradeoff. Betting everything on one model provider looks like the same bet, but three things are different.
    ...
    - **The dependency is younger:** hyperscalers spent two decades reducing their failure rate on traditional platforms; model providers are new and moving fast. Cloud contracts can carry an SLA of 99.9-99.99% per service, while standard-tier model APIs today often publish no comparable availability commitment, and even half a nine of availability gap compounds into hours a month.
    - **The escape hatch is cheaper:** switching clouds is a year of engineering; switching model providers can be hours to days of work.

**The principle:** Inventory the dependencies your redundancy shares (model provider, credentials, billing account) and give each one an owner. A fallback provider is within almost any team's reach; going without one is a decision worth making explicitly.
...
**Grounding:** The gap cannot log itself; what got logged instead are the records around it. February 23, 2026, 12:26 AM: the wish was filed in the operator's own words: "context aware weather report in the morning briefing… if I have a trip coming up it also will give me that." The capability entered the roster only because a human asked; nothing in the system had flagged its absence. Built within the hour. And the measurement half: March 7, a briefing run scored perfect on every machine dimension; the operator docked a point anyway. The docked point's reason is recorded nowhere (the score tables had no field to hold it), which is this mode's thesis in one row: the gap between the mission and the roster lived only in the human who noticed it.

The adjacent literatures own pieces of the detector. Production dialogue systems detect out-of-scope requests one at a time <span class="fk-cite">(Larson et al., 2019, arXiv:1909.02027)</span>, and intent-discovery work mines unhandled asks to expand the supported set <span class="fk-cite">(Zhang, Y. et al., 2022, arXiv:2205.12914)</span>, the demand-side half of this mode's remedy, framed there as service expansion rather than reliability. The agent-fleet literature concedes narrow task coverage but only sees it under benchmark probing <span class="fk-cite">(So et al., 2026, arXiv:2604.18951)</span>, and a 257-paper validation survey finds standing coverage measurement under-developed <span class="fk-cite">(Mirto et al., 2026, arXiv:2607.29405)</span>. What no checked source carries: a declared mission, including the not-yet-built, as the record the roster is diffed against; and the silence itself, where even fallback telemetry goes quiet once the asks stop.
...
*A common parent mechanism.*

**What:** The model states something false with the confidence of something true: a fact, a date, an event that does not exist. This is the industry's most-studied failure, and the toolchain against it is comparatively mature: grounding, retrieval, fact-checking, judge models <span class="fk-cite">(Huang, L. et al., 2023)</span>. It lives in an appendix rather than the numbered catalog because the catalog is for what that toolchain cannot see.

**Why it still leads the chart:** Hallucination is a common parent mechanism behind the action modes. The same invented value that is merely a wrong sentence in chat becomes #28 when the model narrates an action it never took, #13 when it rides a real write, and #32 when it crosses a handoff and poisons the next agent's work. Output-side tooling catches the first case; the other three are why this guide exists. A note on vocabulary: the research names the action-side events as model behaviors, measured on benchmarks: tool selection hallucination, tool content hallucination <span class="fk-cite">(Xu, H. et al., 2024)</span>. This guide keeps those terms and changes their frame: inside a running workflow they are not measurements but live events, and each one has a fate: caught at a gate, executed into state, or inherited by the next agent. The numbered modes are those fates.

**Observed incident:** March 2026, already filed above as the second incident under #22 (Phantom Success): the calendar agent that, finding no events, invented one. Seen at this layer it is plain hallucination. Seen at the catalog's layer it is a faked work product, which is why it carries a number. A read-side hallucination costs a wrong answer on a screen; the write-side versions cost real state. That difference is where the numbered modes begin.

<div class="fk-backmatter">

## References

Most arXiv citations here are preprints; figures quoted from them are their authors' own measurements, not yet peer-reviewed, and are cited for the mechanism they demonstrate rather than the exact rate.
...
- Deshpande, D. et al. (2025). *TRAIL: Trace Reasoning and Agentic Issue Localization*. arXiv:2505.08638.
...
- Microsoft AI Red Team (2026). [*Taxonomy of Failure Modes in Agentic AI Systems*, v2.0](https://cdn-dynmedia-1.microsoft.com/is/content/microsoftcorp/microsoft/bade/documents/products-and-services/en-us/security/Taxonomy-of-Failure-Modes-in-Agentic-AI-Systems-v2-0.pdf) ([announcement](https://www.microsoft.com/en-us/security/blog/2026/06/04/updating-taxonomy-failure-modes-agentic-ai-systems-year-red-teaming-taught-us/)). microsoft.com. <span class="fk-refnote">(The attacker-centric complement: the security axis this catalog excludes)</span>
...
- OWASP GenAI Security Project (2025). [*Top 10 for Agentic Applications for 2026*](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/). genai.owasp.org.
...
- Shah, M. B. et al. (2026). *Characterizing Faults in Agentic AI: A Taxonomy of Types, Symptoms, and Root Causes*. arXiv:2603.06847. <span class="fk-refnote">(Developer-filed framework bugs; the loud-symptom complement to this catalog's silent ones)</span>
...
- Wu, W. (2026). *When Errors Become Narratives: A Longitudinal Taxonomy of Silent Failures in a Production LLM Agent Runtime*. arXiv:2606.14589. <span class="fk-refnote">(The single-agent sibling study; names #22's mechanism "fail-plausible")</span>
...
- Zhu, K. et al. (2025). *Where LLM Agents Fail and How They Can Learn From Failures*. arXiv:2509.25370. <span class="fk-refnote">(Names causal misattribution, #16's model-side case)</span>
...
Jessica Kosturko is an engineering leader with extensive experience leading Site Reliability Engineering teams at enterprise scale. This independently developed catalog applies that reliability perspective to operational evidence from an instrumented agent platform she designed, built, and operates.

Found an error, or have a logged incident that matches or breaks one of these modes? Open an issue at [github.com/jkosturko/agent-failure-key](https://github.com/jkosturko/agent-failure-key).
...
[CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). Cite the version you used. Quoted material from cited works stays under its original authors' rights; this license does not cover it.
